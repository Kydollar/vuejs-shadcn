...
onMounted(async () => {
  ...
  if (inspections.value.length === 0) {
    await qcStore.fetchInspections()
  }
})
</script>
  ...
  <div class="space-y-6">
    ...
    <Card>
      ...
      <CardContent>
        <div class="overflow-x-auto">
          ...
          <div v-if="inspections.length === 0" class="text-center py-8">
            <p class="text-muted-foreground">
              {{ $t('common.noData') }}
            </p>
          </div>
    ...
    <Dialog :open="showDialog" @update:open="showDialog = $event">
      <DialogContent class="max-w-2xl">
        ...
        <form class="space-y-4" @submit.prevent="saveInspection">
          ...
          <div class="space-y-2">
            ...
            <div class="grid grid-cols-2 gap-2">
              <label class="flex items-center space-x-2">
                <input v-model="rejectionReasons" type="checkbox" value="sizeIssue" />
                <span class="text-sm">{{ $t('erp.qc.sizeIssue') }}</span>
              </label>
              <label class="flex items-center space-x-2">
                <input v-model="rejectionReasons" type="checkbox" value="colorIssue" />
                <span class="text-sm">{{ $t('erp.qc.colorIssue') }}</span>
              </label>
              <label class="flex items-center space-x-2">
                <input v-model="rejectionReasons" type="checkbox" value="stitchingDefect" />
                <span class="text-sm">{{ $t('erp.qc.stitchingDefect') }}</span>
              </label>
              <label class="flex items-center space-x-2">
                <input v-model="rejectionReasons" type="checkbox" value="fabricDefect" />
                <span class="text-sm">{{ $t('erp.qc.fabricDefect') }}</span>
              </label>
              <label class="flex items-center space-x-2">
                <input v-model="rejectionReasons" type="checkbox" value="decorationDefect" />
                <span class="text-sm">{{ $t('erp.qc.decorationDefect') }}</span>
              </label>
              <label class="flex items-center space-x-2">
                <input v-model="rejectionReasons" type="checkbox" value="other" />
